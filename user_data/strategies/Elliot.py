...
from freqtrade.strategy import (
    CategoricalParameter, DecimalParameter, IStrategy, IntParameter)
...
def pump_warning2(dataframe, params):
    ...
    df['pct_change_int'] = ((df['pct_change'] > pct_change_max).astype(
        'int') | (df['pct_change'] < pct_change_min).astype('int'))
    df['pct_change_short'] = df['close'].pct_change(
        periods=pct_change_short_timeframe)
    df['pct_change_int_short'] = ((df['pct_change_short'] > pct_change_short_max).astype(
        'int') | (df['pct_change_short'] < pct_change_short_min).astype('int'))
    df['ispumping'] = ((df['pct_change_int'].rolling(
        ispumping_rolling).sum() >= ispumping)).astype('int')
    df['islongpumping'] = ((df['pct_change_int'].rolling(
        islongpumping_rolling).sum() >= islongpumping)).astype('int')
    df['isshortpumping'] = ((df['pct_change_int_short'].rolling(
        isshortpumping_rolling).sum() >= isshortpumping)).astype('int')
    df['recentispumping'] = (df['ispumping'].rolling(recentispumping_rolling).max() > 0) | (df['islongpumping'].rolling(
        recentispumping_rolling).max() > 0) | (df['isshortpumping'].rolling(recentispumping_rolling).max() > 0)
...
def dump_warning(dataframe, buy_threshold):
    # Get recent BTC info
    df_past = dataframe.copy().shift(1)

    # 5m dump protection
    df_past_source = (df_past['open'] + df_past['close'] +
                      df_past['high'] + df_past['low']) / 4        # Get BTC price
    # BTC dump n% in 5 min
    df_threshold = df_past_source * buy_threshold
    # should be positive if dump
    df_past_delta = df_past['close'].shift(1) - df_past['close']
    # Need be larger than 0
    df_diff = df_threshold - df_past_delta
    ...
    df_past_source_1d = (
        df_past_1d['open'] + df_past_1d['close'] + df_past_1d['high'] + df_past_1d['low']) / 4
...
def T3(dataframe, length=5):
    ...
    df['T3Average'] = c1 * df['xe6'] + c2 * \
        df['xe5'] + c3 * df['xe4'] + c4 * df['xe3']
...
class Elliot(IStrategy):
    ...
    # ADIX
    ewo_high_adix = DecimalParameter(
        2.0, 12.0, default=buy_params['ewo_high_adix'], space='buy', optimize=True)
    ewo_high2_adix = DecimalParameter(
        2.0, 12.0, default=buy_params['ewo_high2_adix'], space='buy', optimize=True)
    ewo_high2_adix
    ewo_low_adix = DecimalParameter(
        2.0, 12.0, default=buy_params['ewo_low_adix'], space='buy', optimize=True)
    ewo_low2_adix = DecimalParameter(
        2.0, 12.0, default=buy_params['ewo_low2_adix'], space='buy', optimize=True)
    rsi_buy_adix = IntParameter(
        30, 70, default=buy_params['rsi_buy_adix'], space='buy', optimize=False)
    rsi_buy2_adix = IntParameter(
        30, 70, default=buy_params['rsi_buy2_adix'], space='buy', optimize=False)

    # LAMBO
    lambo2_ema_14_factor = DecimalParameter(
        0.9, 0.99, default=buy_params['lambo2_ema_14_factor'], space='buy', optimize=True)
    lambo2_rsi_4_limit = IntParameter(
        2, 50, default=buy_params['lambo2_rsi_4_limit'], space='buy', optimize=True)
    lambo2_rsi_14_limit = IntParameter(
        2, 50, default=buy_params['lambo2_rsi_14_limit'], space='buy', optimize=True)
    ...
    ewo_low2 = DecimalParameter(-20.0, -8.0,
                                default=buy_params['ewo_low2'], space='buy', optimize=True)
    ewo_high2 = DecimalParameter(
        2.0, 12.0, default=buy_params['ewo_high2'], space='buy', optimize=True)
    ewo_high_2 = DecimalParameter(-6.0, 12.0,
                                  default=buy_params['ewo_high_2'], space='buy', optimize=True)

    rsi_buy = IntParameter(
        10, 80, default=buy_params['rsi_buy'], space='buy', optimize=True)
    rsi_buy2 = IntParameter(
        30, 70, default=buy_params['rsi_buy2'], space='buy', optimize=True)
    ...
    buy_bb_factor = DecimalParameter(
        0.990, 0.999, default=0.995, optimize=False)
    ...
    buy_rsi_local_dip = IntParameter(
        15, 45, default=28, optimize=is_optimize_local_dip)
    ...
    buy_ewo = DecimalParameter(-6.0, 5, default=-
                               5.585, optimize=is_optimize_ewo)
    buy_ema_low = DecimalParameter(
        0.9, 0.99, default=0.942, optimize=is_optimize_ewo)
    buy_ema_high = DecimalParameter(
        0.95, 1.2, default=1.084, optimize=is_optimize_ewo)
    ...
    buy_ema_cofi = DecimalParameter(
        0.94, 1.2, default=0.97, optimize=is_optimize_cofi)
    ...
    buy_ewo_high = DecimalParameter(
        2, 12, default=3.553, optimize=is_optimize_cofi)

    is_optimize_cofi_protection = False
    buy_cofi_cti = DecimalParameter(-0.9, -0.0, default=-
                                    0.5, optimize=is_optimize_cofi_protection)
    buy_cofi_r14 = DecimalParameter(-100, -44, default=-60,
                                    optimize=is_optimize_cofi_protection)

    is_optimize_gumbo = False
    buy_gumbo_ema = DecimalParameter(
        0.9, 1.2, default=0.97, optimize=is_optimize_gumbo)
    buy_gumbo_ewo_low = DecimalParameter(-12.0, 5,
                                         default=-5.585, optimize=is_optimize_gumbo)

    is_optimize_gumbo_protection = False
    buy_gumbo_cti = DecimalParameter(-0.9, -0.0, default=-
                                     0.5, optimize=is_optimize_gumbo_protection)
    buy_gumbo_r14 = DecimalParameter(-100, -44, default=-60,
                                     optimize=is_optimize_gumbo_protection)
    ...
    buy_sqzmom_ewo = DecimalParameter(-12, 12, default=0,
                                      optimize=is_optimize_sqzmom_protection)
    buy_sqzmom_r14 = DecimalParameter(-100, -22, default=-50,
                                      optimize=is_optimize_sqzmom_protection)
    ...
    buy_nfix_49_cti = DecimalParameter(-0.9, -0.0, default=-
                                       0.5, optimize=is_optimize_nfix_49_protection)
    buy_nfix_49_r14 = DecimalParameter(-100, -44, default=-60,
                                       optimize=is_optimize_nfix_49_protection)

    is_optimize_btc_safe = False
    buy_btc_safe = IntParameter(-300, 50, default=-
                                200, optimize=is_optimize_btc_safe)
    buy_btc_safe_1d = DecimalParameter(-0.075, -0.025,
                                       default=-0.05, optimize=is_optimize_btc_safe)
    ...
    buy_rsi_15 = DecimalParameter(
        20.0, 36.0, default=28.0, optimize=is_optimize_ctt15_protection)
    ...
    buy_25_rsi_4 = DecimalParameter(
        26.0, 40.0, default=38.0, optimize=is_optimize_ctt25_protection)
    buy_25_cti = DecimalParameter(-0.99, -0.4, default=-
                                  0.76, optimize=is_optimize_ctt25_protection)
    ...
    buy_ewo_13 = DecimalParameter(-14.0, -7.0, default=-11.4,
    ...
    buy_rsi_fast_ewo_2 = IntParameter(
        15, 50, default=45, optimize=is_optimize_ewo_2)
    buy_rsi_ewo_2 = IntParameter(
        15, 50, default=35, optimize=is_optimize_ewo_2)
    ...
    buy_ewo_high_2 = DecimalParameter(
        2, 12, default=4.179, optimize=is_optimize_ewo_2)
    ...
    buy_12_protection__close_above_ema_fast_len = CategoricalParameter(
        ["12", "20", "26", "50", "100", "200"], default="200", space='buy', optimize=False, load=True)
    ...
    sell_cmf = DecimalParameter(-0.4, 0.0, default=0.0,
                                optimize=is_optimize_sell_stoploss)
    ...
    sell_bleeding_cti = DecimalParameter(-0.9, -0.0,
                                         default=-0.5, optimize=is_optimize_bleeding)
    sell_bleeding_r14 = DecimalParameter(-100, -44,
                                         default=-60, optimize=is_optimize_bleeding)
    sell_bleeding_volume_factor = DecimalParameter(
        1, 2.5, default=1.0, optimize=is_optimize_bleeding)

    is_optimize_cti_r = False
    sell_cti_r_cti = DecimalParameter(
        0.55, 1, default=0.5, optimize=is_optimize_cti_r)
    sell_cti_r_r = DecimalParameter(-15, 0,
                                    default=-20, optimize=is_optimize_cti_r)
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h['sma_200_dec_20'] = informative_1h['sma_200'] < informative_1h['sma_200'].shift(
            20)
        informative_1h['sma_200_dec_24'] = informative_1h['sma_200'] < informative_1h['sma_200'].shift(
            24)
        ...
        # CRSI (3, 2, 100)
        crsi_closechange = informative_1h['close'] / \
            informative_1h['close'].shift(1)
        crsi_updown = np.where(crsi_closechange.gt(
            1), 1.0, np.where(crsi_closechange.lt(1), -1.0, 0.0))
        informative_1h['crsi'] = (ta.RSI(informative_1h['close'], timeperiod=3) + ta.RSI(
            crsi_updown, timeperiod=2) + ta.ROC(informative_1h['close'], 100)) / 3
        ...
        informative_1h['bb_width'] = (
            (informative_1h['bb_upperband2'] - informative_1h['bb_lowerband2']) / informative_1h['bb_middleband2'])
        ...
        informative_1h['rocr'] = ta.ROCR(
            informative_1h['ha_close'], timeperiod=168)
        ...
        informative_1h['low_5'] = informative_1h['low'].shift().rolling(
            5).min()
        informative_1h['safe_dump_50'] = ((informative_1h['hl_pct_change_5'] < 0.66) | (
    ...
    def informative_15m_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_15m['sma_200_dec_20'] = informative_15m['sma_200'] < informative_15m['sma_200'].shift(
            20)
        ...
        bb_40_std2 = qtpylib.bollinger_bands(
            informative_15m['close'], window=40, stds=2)
        informative_15m['bb40_2_low'] = bb_40_std2['lower']
        informative_15m['bb40_2_mid'] = bb_40_std2['mid']
        informative_15m['bb40_2_delta'] = (
            bb_40_std2['mid'] - informative_15m['bb40_2_low']).abs()
        informative_15m['closedelta'] = (
            informative_15m['close'] - informative_15m['close'].shift()).abs()
        informative_15m['tail'] = (
            informative_15m['close'] - informative_15m['bb40_2_low']).abs()
        ...
        # CCI
        informative_15m['cci'] = ta.CCI(
            informative_15m, source='hlc3', timeperiod=20)

        # CRSI (3, 2, 100)
        crsi_closechange = informative_15m['close'] / \
            informative_15m['close'].shift(1)
        crsi_updown = np.where(crsi_closechange.gt(
            1), 1.0, np.where(crsi_closechange.lt(1), -1.0, 0.0))
        informative_15m['crsi'] = (ta.RSI(informative_15m['close'], timeperiod=3) + ta.RSI(
            crsi_updown, timeperiod=2) + ta.ROC(informative_15m['close'], 100)) / 3
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # pump detector
        dataframe['pump'] = pump_warning(
            dataframe, perc=int(self.max_change_pump))  # 25% di pump
        ...
        # BB 20 - STD3
        bb_20_std3 = qtpylib.bollinger_bands(
            qtpylib.typical_price(dataframe), window=20, stds=3)
        ...
        dataframe['bb_delta_cluc'] = (
            dataframe['bb_middleband2_40'] - dataframe['bb_lowerband2_40']).abs()
        dataframe['ha_closedelta'] = (
            dataframe['ha_close'] - dataframe['ha_close'].shift()).abs()
        ...
        dataframe['kc_upperband_28_1'] = dataframe['sma_28'] + \
            dataframe['range_ma_28']
        dataframe['kc_lowerband_28_1'] = dataframe['sma_28'] - \
            dataframe['range_ma_28']

        # KC 20
        dataframe['range_ma_20'] = ta.SMA(dataframe['trange'], 20)
        dataframe['kc_upperband_20_2'] = dataframe['sma_20'] + \
            dataframe['range_ma_20'] * 2
        dataframe['kc_lowerband_20_2'] = dataframe['sma_20'] - \
            dataframe['range_ma_20'] * 2
        dataframe['kc_bb_delta'] = (dataframe['kc_lowerband_20_2'] -
                                    dataframe['bb_lowerband2']) / dataframe['bb_lowerband2'] * 100

        # Linreg
        dataframe['hh_20'] = ta.MAX(dataframe['high'], 20)
        dataframe['ll_20'] = ta.MIN(dataframe['low'], 20)
        dataframe['avg_hh_ll_20'] = (
            dataframe['hh_20'] + dataframe['ll_20']) / 2
        dataframe['avg_close_20'] = ta.SMA(dataframe['close'], 20)
        dataframe['avg_val_20'] = (
            dataframe['avg_hh_ll_20'] + dataframe['avg_close_20']) / 2
        ...
        dataframe['ema_offset_buy2'] = ta.EMA(dataframe, int(
            self.base_nb_candles_buy_ema2.value)) * self.low_offset_ema2.value
        ...
        dataframe['trima_offset_buy2'] = ta.TRIMA(dataframe, int(
            self.base_nb_candles_buy_trima2.value)) * self.low_offset_trima2.value

        # ZEMA
        dataframe['zema_offset_buy'] = zema(dataframe, int(
            self.base_nb_candles_buy_zema.value)) * self.low_offset_zema.value
        dataframe['zema_offset_buy2'] = zema(dataframe, int(
            self.base_nb_candles_buy_zema2.value)) * self.low_offset_zema2.value
        ...
        dataframe['rsi_slow_descending'] = (
            dataframe['rsi_slow'] < dataframe['rsi_slow'].shift()).astype('int')
    ...
    def populate_btc_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative = self.dp.get_pair_dataframe(
            'BTC/USDT', timeframe=self.timeframe)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_can_buy_smooth_ha = (
            ...
            (dataframe['Smooth_HA_O'].shift(1) <
             dataframe['Smooth_HA_H'].shift(1))
        )
        ...
        is_pair_safe = (
            ...
            (dataframe['pair_5m_1d_diff'] >
             dataframe['pair_1d'] * self.buy_btc_safe_1d.value)
        )
        ...
        is_ewo_low = (
            ...
            (dataframe['volume'] > 0) &
            (dataframe['close'] < (
                dataframe[f'ma_sell_{self.base_nb_candles_sell3.value}'] * self.high_offset3.value))
        )
        ...
        conditions.append(
            is_gumbo)                                                # ~2.63 / 90.6% / 41.49%      D
        dataframe.loc[is_gumbo, 'buy_tag'] += 'gumbo '

        # ~0.78 / 92.0 % / 37.41%     D
        conditions.append(is_nfix_46)
